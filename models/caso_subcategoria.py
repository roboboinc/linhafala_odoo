...
class CasoSubcategoria(models.Model):
    ...
    def _backfill_case_records(self):
        ...
        if not self._column_exists(case_model._table, 'secundary_case_type_snapshot'):
            return
        records = case_model.search([
            ('secundary_case_type_snapshot', '=', False),
            ('secundary_case_type', '!=', False),
        ])
        for record in records:
            record.with_context(
                skip_case_person_role_validation=True,
                skip_case_priority_backfill_validation=True,
            ).write({'secundary_case_type_snapshot': record.secundary_case_type.name})
